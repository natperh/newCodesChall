...
6. La transacción debe ser rechazada si el monto de la transacción es mayor que el límite diario.
7. La transacción debe ser aprobada si el monto de la transacción no es mayor que el límite diario.
...
* **INICIO**:
 ...
 5. Se verifica si el `MONTO-TRANSACCION` es mayor que el `LIMITE-DIARIO`.
...
| --- | --- | --- |
| Ingresar número de tarjeta | 100% | 100% (Test: testProcesarTransaccion, Gherkin: Escenario: Ingresar número de tarjeta válido) |
| Ingresar cuenta bancaria | 100% | 100% (Test: testProcesarTransaccion, Gherkin: Escenario: Ingresar cuenta bancaria válida) |
| Ingresar RFC | 100% | 100% (Test: testProcesarTransaccion, Gherkin: Escenario: Ingresar RFC válido) |
| Ingresar monto de transacción | 100% | 100% (Test: testProcesarTransaccion, Gherkin: Escenario: Ingresar monto de transacción válido) |
| Verificar límite diario | 100% | 100% (Test: testProcesarTransaccionConMontoMayorAlLimite, Gherkin: Escenario: Ingresar monto de transacción mayor que el límite diario) |
| Mostrar resultado de la transacción | 100% | 100% (Test: testProcesarTransaccion, Gherkin: Escenario: Mostrar resultado de la transacción) |
| **Totales** | **100%** | **100%** |
...
Escenario: Ingresar monto de transacción mayor que el límite diario
  Dado que el monto de la transacción es "10001.00"
  Cuando se ingresa el monto de la transacción
  Entonces el sistema debe rechazar la transacción

Escenario: Ingresar monto de transacción menor o igual que el límite diario
  Dado que el monto de la transacción es "1000.00"
  Cuando se ingresa el monto de la transacción
